...
3. Create a failover group with a mirror disk resource, mirroring the 5GB drives on each server (e.g. drive F:). Also add a script resource to maintain snapshots on the mirror disk in the event that the drive is failed over to the standby server and then failed back to the primary server.
